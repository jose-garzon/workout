# profile-edit Specification (delta)
...
**Reason:** The drawer is deleted. Swipe-right, backdrop click, the header close
(X) button, and Escape are all drawer-specific dismissal mechanics that have no
meaning for a full page — a page is left by navigating, not by dismissing an
overlay, and there is no backdrop, no X, and no overlay for Escape to close.

**Migration:** Discarding an edit is now covered by the new requirement "Leaving
the edit page without saving discards the edits": navigating away from
`/profile/edit` (browser/system back, or any other navigation) leaves saved data
unchanged, and the page re-seeds from the saved records on the next visit. The
guarantee the four affordances existed to provide — "a discarded edit never
survives" — is preserved; only its trigger changed.
