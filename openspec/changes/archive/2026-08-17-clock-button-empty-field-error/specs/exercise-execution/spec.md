...
- FROM: `### Requirement: Per-exercise view shows the plan and a weight field`
- TO: `### Requirement: Per-exercise view shows the plan and the set's reps and weight fields`
...
**Migration:** Replaced by "Reps and weight are entered per set and both gate the start" — no stored data changes; a set's recorded reps are now the user's confirmed entry (see the `session-tracking` delta) instead of the plan's reps for that set index.

**Reason:** Superseded and renamed by "Reps and weight are entered per set and both gate
the start". Both the name and the body are now false: the per-exercise view asks the user
for reps per set (it has since reps entry shipped), so "a single weight field" and "the
user is not asked to enter reps per series" no longer describe the product. Its two
surviving scenarios — the weight in effect being recorded per series, and an edit between
sets being captured per set — are preserved verbatim-in-substance in the replacement,
with reps added alongside weight.
