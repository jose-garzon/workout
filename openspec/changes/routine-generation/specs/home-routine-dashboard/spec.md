...
The home surface once a user is onboarded: an identity header (greeting name, a
badge of their goal, and a short motivational line), a per-day summary of the
active routine when one exists, a neutral empty state when none does, and tapping
a day to open workout mode. This spec owns *what home shows*; the composer and
generation behavior are the `routine-generation` capability's.
