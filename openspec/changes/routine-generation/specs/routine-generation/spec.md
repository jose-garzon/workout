...
Turning a typed prompt (plus the user's saved profile and goals) into a
structured routine via the AI backend, and making the wait legible: an animated
building indicator and a live summary of the model's thinking while it works.
This spec states only *what*; streaming mechanics and the proxy contract are the
design's.
