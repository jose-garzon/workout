...
**Second non-obvious bit (verified 2026-08-05, evidence-based-routine-prompts):** rule 4 is scoped to `src/app/api/**/route.ts` and is **not transitive** — it checks that file's own import statements only. `route.ts` already imports `api/ai/openrouter.ts`, which imports `./prompt`, none of which Biome inspects for rule 4. **Consequence: adding a new sibling under `modules/routine-generation/api/ai/` costs ZERO config change and needs no new fixture** — the `modules/*/api/**` override only bans upward `logic/`/`ui/` imports, and depcruise rule 3 is cross-*feature* only. Server-safety of those siblings (pure, no `shared/db`) is **discipline, not tooling** — say so in design.md rather than assuming the firewall catches it.
